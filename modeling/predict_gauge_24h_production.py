...
def build_summary(pred_df: pd.DataFrame, training_summary: dict, feature_columns: list[str]) -> dict:
    ...
    return {
        ...
        "min_forecast_timestamp_utc": valid_forecasts.min().isoformat() if not valid_forecasts.empty else None,
        "max_forecast_timestamp_utc": valid_forecasts.max().isoformat() if not valid_forecasts.empty else None,
    }
