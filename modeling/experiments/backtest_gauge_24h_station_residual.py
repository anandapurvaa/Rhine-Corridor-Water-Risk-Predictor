    ...
    preprocessor = ColumnTransformer(
        transformers=[
            (
                ...
                ["station_name", "timeseries_name", "source"],
            ),
            (
                "num",
                Pipeline(
...
def build_residual_train_frame(meta_train: pd.DataFrame, base_pred_train: np.ndarray) -> pd.DataFrame:
    residual_train = meta_train.copy()
    residual_train["base_prediction"] = base_pred_train
    residual_train["base_residual"] = residual_train[TARGET_COLUMN] - residual_train["base_prediction"]
    return residual_train


def build_residual_test_frame(meta_test: pd.DataFrame, base_pred_test: np.ndarray) -> pd.DataFrame:
    residual_test = meta_test.copy()
    residual_test["base_prediction"] = base_pred_test
    return residual_test
...
def main():
    ...
    for fold_num, (train_idx, test_idx) in enumerate(tscv.split(X), start=1):
        ...
        residual_train = build_residual_train_frame(meta_train, base_pred_train)
        residual_test = build_residual_test_frame(meta_test, base_pred_test)

        residual_model = build_residual_pipeline()
        residual_model.fit(
            residual_train.drop(columns=["base_residual"]),
            residual_train["base_residual"],
        )

        pred_residual_test = residual_model.predict(residual_test)
        final_pred_test = base_pred_test + pred_residual_test
        ...
        fold_df["predicted_residual_adjustment"] = pred_residual_test
    ...
    base_pred_full = full_global_model.predict(X)
    residual_full = df[["timestamp_utc", "station_name", "timeseries_name", "source"] + NUMERIC_COLUMNS + [TARGET_COLUMN]].copy()
    residual_full["base_prediction"] = base_pred_full
    residual_full["base_residual"] = residual_full[TARGET_COLUMN] - residual_full["base_prediction"]

    full_residual_model = build_residual_pipeline()
    full_residual_model.fit(
        residual_full.drop(columns=["base_residual"]),
        residual_full["base_residual"],
    )
    ...
    joblib.dump(full_global_model, OUTPUT_DIR / "gauge_24h_station_residual_global_model.joblib")
    joblib.dump(full_residual_model, OUTPUT_DIR / "gauge_24h_station_residual_adjuster.joblib")

    with open(OUTPUT_DIR / "gauge_24h_station_residual_summary.json", "w", encoding="utf-8") as f:
        json.dump(summary, f, indent=2)

    fold_metrics_df.to_csv(OUTPUT_DIR / "gauge_24h_station_residual_fold_metrics.csv", index=False)
    predictions_df.to_csv(OUTPUT_DIR / "gauge_24h_station_residual_predictions.csv", index=False)
    station_metrics_df.to_csv(OUTPUT_DIR / "gauge_24h_station_residual_station_metrics.csv", index=False)
